...
def usb_presence(lines: list[str]) -> dict[str, bool]:
    joined = "\n".join(lines).lower()
    return {
        "bootloader": "2e8a:000f" in joined or "2e8a:0003" in joined,
        "application": "1050:0407" in joined,
        "any_picokey": "2e8a:" in joined or "1050:0407" in joined,
    }


def reboot_to_recovery() -> dict[str, Any]:
    try:
        from picokey import PicoKey
    except Exception as exc:
        raise HTTPException(501, f"picokey APDU backend unavailable: {exc}") from exc
    try:
        pk = PicoKey()
        pk.reboot(bootsel=True)
        log("requested reboot to firmware install mode")
        return {"ok": True, "message": "reboot requested"}
    except Exception as exc:
        raise HTTPException(500, f"reboot request failed: {exc}") from exc
...
function setText(id,value){let el=$(id);if(el)el.textContent=value??'—'}function clearDevice(){['infoType','infoProduct','infoVersion','infoBoardStatus','infoConn','infoSerial','flashFree','flashTotal','flashFiles','flashUsed','flashSize','firmwareSize','flashMax'].forEach(id=>setText(id,'—'));if($('usageFill'))$('usageFill').style.width='0%'}
async function refresh(){let s=await fetch('/status').then(r=>r.json());let connected=!!s.any_picokey;$('sideBadge').textContent=connected?'Online':'Offline';$('sideBadge').className='pill '+(connected?'ok':'off');$('devTime').textContent=connected?new Date().toISOString().slice(0,19).replace('T',' '):'—';clearDevice();if(s.board){setText('boardName',connected?s.board.name:'—');setText('infoPlatform',connected?(s.board.platform||'').toUpperCase():'—')}if(s.bootloader){setText('infoType','PICOKEY');setText('infoProduct','Firmware install mode');setText('infoConn','PICOBOOT');setText('infoBoardStatus','Recovery Mode')}else if(s.application){setText('infoType','PICOKEY');setText('infoProduct','Connected');setText('infoConn','SMARTCARD');setText('infoBoardStatus','Detected')}await refreshBoards();await refreshVendors();await refreshImages()}
...
def status() -> dict[str, Any]:
    usb = usb_devices()
    presence = usb_presence(usb)
    base = {"board": selected_board(), "images": len(list_images()), "usb": usb, **presence}
    try:
        pb = open_boot()
        try:
            return {**base, "bootloader": True, "any_picokey": True, "memory": str(pb.memory)}
        finally:
            pb.close()
    except Exception as exc:
